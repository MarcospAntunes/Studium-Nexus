...
function RenderBhaskaraExplanation({
  a,
  b,
  delta,
}: renderBhaskaraExplanationProps): JSX.Element | undefined {
  if (typeof a === "number" && typeof b === "number" && delta !== undefined) {
    ...
    if (delta === 0) {
      return (
        <>
          <li>
            Neste caso, terá apenas uma raíz que devemos calcular da seguinte
            forma:
    ...
    if (delta > 0) {
      ...
      return (
        <>
          <li>
            Neste caso, terá <strong>duas raízes</strong> que devemos calcular
            da seguinte forma:
            <div className="formulaContent">
              <p className="fraction">
                <br />
          ...
          <li>
            Com isso concluimos que o resultado da equação possui duas raízes
            que são:
            <p className="formulaContent">
              <span>
                x<sub>1</sub> = {x1}
